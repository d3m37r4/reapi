...
void CBasePlayer_Drop_Afk(IReGameHook_CBasePlayer_Drop_Afk *chain, CBasePlayer *pthis)
{
	auto original = [chain](int _pthis)
	{
		return chain->callNext(getPrivate<CBasePlayer>(_pthis));
	...
	callVoidForward(RG_CBasePlayer_Drop_Afk, original, indexOfEdict(pthis->pev));
}
